...
const Products = ({ props }) => {
  ...
  async function getProducts() {
    try {
      ...
      console.log(req);
      const response = await fetch(`${settings.connectionString}/api/category_product_read_by_name.php`, {
        method: 'POST',
  ...
  const changeCategory = (categoryId) => event => {
    ...
    UIkit.modal("#modal_categorias_search").hide();
    history.push('/productos/' + categoryId);
  }
  const orderBy = (event) => {
